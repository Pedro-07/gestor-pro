...
export default function VendaDetalhePage() {
  ...
  async function handleExportPDF() {
    ...
    for (const item of venda.itens) { if (y > 260) { pdf.addPage(); y = 20 }; pdf.text(item.produtoNome.substring(0, 38), 15, y); pdf.text(usarTamanhos ? item.tamanho : '', 110, y); pdf.text(String(item.quantidade), 125, y); pdf.text(formatCurrency(item.precoUnitario), 140, y); pdf.text(formatCurrency(item.subtotal), 168, y); y += 6 }
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      ...
      <Card><CardHeader><CardTitle className="text-base">Itens da Venda</CardTitle></CardHeader><CardContent><div className="space-y-2">{venda.itens.map((item, i) => (<div key={i} className="flex items-center justify-between text-sm py-1 border-b last:border-0"><div><p className="font-medium">{item.produtoNome}</p><p className="text-xs text-muted-foreground">{usarTamanhos ? `${item.tamanho} × ` : ''}{item.quantidade} — {formatCurrency(item.precoUnitario)}/un</p></div><p className="font-semibold">{formatCurrency(item.subtotal)}</p></div>))}<div className="flex justify-between font-bold pt-2 text-lg"><span>Total</span><span>{formatCurrency(venda.total)}</span></div></div></CardContent></Card>
